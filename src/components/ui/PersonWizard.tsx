...
export default function PersonWizard({ onAddPerson, onSkip }: PersonWizardProps) {
    ...
    if (step < totalSteps) {
      ...
      const person: Person = {
        ...
        incomes: [{
          ...
          custom_tp_rate: pensionType === 'Annat' && tpInputType === 'percentage' ? (customTpRate === '' ? undefined : customTpRate) : undefined,
                ...
                {tpInputType === 'percentage' && (
                  <div>
                    ...
                    <Input
                      ...
                      type="number"
                      value={customTpRate}
                      onChange={(e) => setCustomTpRate(e.target.value === '' ? '' : Number(e.target.value) / 100)}
                      min="0"
                      max="100"
                      step="0.1"
                      placeholder="10"
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 3:
      case 4:
      case 5: return true; // Pensionsavtalswizard steg
      case 6: return true; // Pensionsavtal anpassning
      case 7: return true; // Övrigt sparande
      default: return true; // Sammanfattning
    }
  };

  return (
    <Dialog open={true} onOpenChange={(open) => !open && onSkip()}>
      <DialogContent className="max-w-[95vw] sm:max-w-[640px] rounded-2xl overflow-hidden bg-white mx-2 sm:mx-auto">
        <div className="bg-gradient-to-br from-primary/5 via-accent/5 to-primary/10 p-4 sm:p-5 border-b border-primary/10">
          <DialogHeader className="gap-2">
            <DialogTitle className="flex items-center gap-3 font-serif text-primary text-lg sm:text-xl">
              ...
              Lägg till person i hushållet
            </DialogTitle>
            <DialogDescription className="text-primary/70 text-sm sm:text-base">
              Vi hjälper dig att lägga till en person med alla deras inkomster och pensionsavtal
            </DialogDescription>
          </DialogHeader>
        </div>
