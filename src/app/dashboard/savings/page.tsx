...
import { TrendingUp, Sparkles, Target, Info, Calculator, Plus, X, ArrowLeft } from 'lucide-react';
...
// Använder samma logik som parseApy i calc.ts
...
function calculateWeightedReturnForStocks(assets: Asset[]): number {
  const stockAssets = assets
    .filter(a => a.category === 'Fonder & Aktier')
    .map(a => ({
  ...
  if (stockAssets.length === 0) {
    console.debug('calculateWeightedReturnForStocks: Inga aktier/fonder hittades, använder fallback 7%');
    return 0.07; // Fallback
  }
  
  const total = stockAssets.reduce((sum, x) => sum + x.value, 0);
  if (total <= 0) {
    console.debug('calculateWeightedReturnForStocks: Total värde <= 0, använder fallback 7%');
    return 0.07; // Fallback
  }
  
  const weightedSum = stockAssets.reduce((sum, x) => sum + x.value * (x.apy as number), 0);
  const result = weightedSum / total;
  console.debug('calculateWeightedReturnForStocks:', {
    stockAssetsCount: stockAssets.length,
    totalValue: total,
    weightedSum,
    result: result * 100 + '%',
    stockAssetsDetails: stockAssets.map(s => ({ value: s.value, apy: s.apy }))
  });
  return result;
}

function calculateWeightedReturnForAllAssets(assets: Asset[]): number {
  ...
  if (validAssets.length === 0) {
    console.debug('calculateWeightedReturnForAllAssets: Inga tillgångar med giltig APY, använder fallback 7%');
    return 0.07; // Fallback
  }
  
  const total = validAssets.reduce((sum, x) => sum + x.value, 0);
  if (total <= 0) {
    console.debug('calculateWeightedReturnForAllAssets: Total värde <= 0, använder fallback 7%');
    return 0.07; // Fallback
  }
  
  const weightedSum = validAssets.reduce((sum, x) => sum + x.value * (x.apy as number), 0);
  const result = weightedSum / total;
  console.debug('calculateWeightedReturnForAllAssets:', {
    validAssetsCount: validAssets.length,
    totalValue: total,
    weightedSum,
    result: result * 100 + '%',
    validAssetsDetails: validAssets.map(s => ({ value: s.value, apy: s.apy }))
  });
  return result;
}
...
export default function SavingsPage() {
  ...
  // Hämta data från hushåll
  const { assets, liabilities, persons, totalNetWorth } = useMemo(() => {
    if (!draftHousehold) {
      return {
        assets: [] as Asset[],
    ...
    return { assets, liabilities, persons, totalNetWorth };
  }, [draftHousehold]);
  ...
  const [useInvestedCapital, setUseInvestedCapital] = useState(true); // Default till investerat kapital
  ...
  const defaultStartCapitalMax = useMemo(() => {
    if (totalNetWorth > 900000) {
      return Math.ceil(totalNetWorth * (4/3)); // 1 + 1/3 = 4/3
    }
    return 1000000;
  }, [totalNetWorth]);

  const defaultMonthlySavingsMax = useMemo(() => {
    if (totalMonthlySavings > 20000) {
      return Math.ceil(totalMonthlySavings * (4/3)); // 1 + 1/3 = 4/3
    }
    return 25000;
  }, [totalMonthlySavings]);

  // Beräkna investerat kapital (fonder & aktier)
  const investedCapital = useMemo(() => {
    return assets
      .filter(asset => asset.category === 'Fonder & Aktier')
  ...
  const autoReturns = useMemo(() => {
    ...
    if (useAutoMode && useInvestedCapital) {
      ...
      // Detta är specifikt för sparandekalkylatorn och skiljer sig från calculateAutoReturns
      // som används av FIRE-simulatorn och behöver separera tillgängligt kapital och pension
      const allAssets = assets; // Inkludera alla tillgångar, inklusive pension
      const weightedReturn = calculateWeightedReturnForAllAssets(allAssets);
      
      console.debug('Auto returns - Hela nettoförmögenheten (inkl. pension):', {
        nomAvailable: baseReturns.nomAvailable,
        nomAvailablePercent: baseReturns.nomAvailable * 100,
        calculatedWeightedReturn: weightedReturn,
        calculatedWeightedReturnPercent: weightedReturn * 100,
        totalAssets: assets.length,
        allAssets: allAssets.map(a => ({ 
          category: a.category, 
          value: a.value, 
          expected_apy: a.expected_apy,
          parsedApy: parseApy(a.expected_apy)
        }))
      });
      
      // Använd beräknat värde om det är giltigt, annars fallback till baseReturns
      if (!isNaN(weightedReturn) && isFinite(weightedReturn) && weightedReturn > 0) {
        const realReturnAll = ((1 + weightedReturn) / (1 + 0.02)) - 1;
        return {
          ...baseReturns,
          nomAvailable: weightedReturn,
          realReturnAvailable: realReturnAll
        };
      }
  ...
  const effectiveReturnNominal = useAutoMode
    ? autoReturns.nomAvailable
    : dSliderReturn[0] / 100;
  ...
  const effectiveStartCapital = useAutoMode 
    ? availableStartCapital 
    : dSliderStartCapital[0];
  ...
  // Synkronisera sliderReturn när autoReturns.nomAvailable ändras i auto-läget
  // Detta säkerställer att när man växlar mellan investerat kapital och nettoförmögenhet i auto-läget,
  ...
  useEffect(() => {
    // Förhindra oändliga loopar genom att ignorera uppdateringar när vi redan uppdaterar
    if (isUpdatingSliderReturnRef.current) {
      return;
    }
    
    if (useAutoMode) {
      const newReturnValue = autoReturns.nomAvailable * 100;
      ...
      if (valueChanged || investedCapitalChanged) {
        ...
        try {
          ...
          console.warn('Error updating sliderReturn:', error);
        } finally {
          // Reset flag efter en kort delay för att tillåta state-uppdateringen att slutföras
          setTimeout(() => {
            isUpdatingSliderReturnRef.current = false;
          }, 50);
        }
      }
    } else {
  ...
  // Initialisera debouncedChartData med första chartData
  useEffect(() => {
    if (debouncedChartData.length === 0 && chartData.length > 0) {
      setDebouncedChartData(chartData);
    }
  }, [chartData, debouncedChartData.length]);
  ...
  useEffect(() => {
    ...
    // Om användaren inte drar, uppdatera direkt
  ...
  useEffect(() => {
    ...
    if (isAnimatingRef.current && animationRefs.current['main']) {
      try {
        ...
        console.warn('Error canceling animation when mode changes:', error);
      }
    }
    ...
    animationTimeoutRef.current = setTimeout(() => {
      ...
      if (difference > 100) {
        ...
        const animate = (currentTime: number) => {
          ...
          try {
            setAnimatedAmounts(prev => {
              // Förhindra oändliga loopar genom att kontrollera om värdet faktiskt har ändrats
              if (Math.abs(prev.main - current) < 0.01) {
                return prev;
              }
              return { ...prev, main: current };
            });
          } catch (error) {
            console.warn('Animation error caught, stopping animation:', error);
          ...
          if (progress < 1) {
            ...
            try {
              setAnimatedAmounts(prev => {
                if (prev.main === currentTargetValue) {
                  return prev;
                }
                return { ...prev, main: currentTargetValue };
              });
            } catch (error) {
              console.warn('Animation final state update error caught:', error);
            }
          }
        };
  ...
  // Scroll till sparandekortet när man kommer tillbaka
  const handleBack = () => {
    router.push('/dashboard');
    setTimeout(() => {
      const savingsCard = document.getElementById('savings-card');
      if (savingsCard) {
        savingsCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }, 100);
  };
    ...
    <div className="min-h-screen bg-[var(--surface-bg)] py-4 md:py-8 px-4 md:px-6">
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="bg-white/90 backdrop-blur-sm p-4 md:p-6 rounded-2xl border border-slate-200/40 mb-4 md:mb-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            ...
            >
              ...
              <span className="hidden sm:inline">Tillbaka till översikt</span>
        ...
        <div className="space-y-4 md:space-y-6">
          {/* Info-sektion */}
          <div className="bg-white/70 backdrop-blur-sm p-4 md:p-5 rounded-xl border border-slate-200/40 shadow-subtle">
            <div className="flex items-start gap-2 md:gap-3">
              <Info className="w-4 h-4 md:w-5 md:h-5 text-accent mt-0.5 flex-shrink-0" />
          ...
          <div className="bg-white/70 backdrop-blur-sm p-4 md:p-5 rounded-xl border border-slate-200/40 shadow-subtle">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                <span className="text-xs text-primary/80 font-medium whitespace-nowrap">Manuell</span>
              <Switch
                checked={useAutoMode}
                onCheckedChange={(checked) => {
                  if (!checked) {
                    // När man växlar till manuell, behåll det senaste värdet från auto-läget
                    setSliderStartCapital([availableStartCapital]);
                    setSliderMonthlySavings([totalMonthlySavings]);
                    setSliderReturn([autoReturns.nomAvailable * 100]);
                  }
                  setUseAutoMode(checked);
                  // Behåll useInvestedCapital-värdet när man växlar från auto till manuell
                }}
              />
                <span className="text-xs text-primary/80 font-medium whitespace-nowrap">Auto</span>
              </div>
            </div>
            ...
            {useAutoMode && (
              <div className="mt-4 pt-4 border-t border-slate-200/40">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                    <span className="text-xs text-primary/80 font-medium whitespace-nowrap">Nettoförmögenhet</span>
                    <Switch
                      checked={useInvestedCapital}
                      onCheckedChange={(checked) => {
                        setUseInvestedCapital(checked);
                        // Uppdatera sliderStartCapital när man växlar mellan investerat kapital och nettoförmögenhet i auto-läget
                        if (useAutoMode) {
                          const newValue = checked ? investedCapital : totalNetWorth;
                          setSliderStartCapital([newValue]);
                        }
                      }}
                    />
                    <span className="text-xs text-primary/80 font-medium whitespace-nowrap">Investerat kapital</span>
                  </div>
                </div>
                <div className="mt-2 text-xs text-primary/60">
                  {useInvestedCapital ? (
                    <>Visar: <strong>{formatCurrency(investedCapital)}</strong> (investerat kapital på börsen)</>
                  ) : (
                    <>Visar: <strong>{formatCurrency(totalNetWorth)}</strong> (hela nettoförmögenheten)</>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-primary/60 mb-3">
                  {useAutoMode ? (
                    <>
                      Auto: {formatCurrency(availableStartCapital)} 
                ...
                <div className="flex items-center gap-2 md:gap-3 mb-2">
                  <Input
                    ...
                    pattern="[0-9]*"
                    value={useAutoMode 
                      ? (effectiveStartCapital === 0 ? '' : Math.floor(effectiveStartCapital).toString())
                      : (sliderStartCapital[0] === 0 ? '' : Math.floor(sliderStartCapital[0]).toString())
                    }
                    onChange={(e) => {
                      if (!useAutoMode) {
                        ...
                        if (val === '') {
                          ...
                          if (!isNaN(num)) {
                            ...
                            setDynamicStartCapitalMax(Math.max(defaultStartCapitalMax, Math.ceil(clampedValue * 1.2)));
                          }
                        }
                      }
                    }}
                    placeholder="0"
                    disabled={useAutoMode}
                    min={0}
                    max={9999999999999}
                ...
                <Slider
                  value={useAutoMode ? [effectiveStartCapital] : sliderStartCapital}
                  onValueChange={(vals) => {
                    if (!useAutoMode && !isSlidingRef.current) {
              ...
              <div>
                ...
                <div className="flex items-center gap-2 md:gap-3 mb-2">
                  <Input
                    ...
                    inputMode="numeric"
                    value={useAutoMode 
                      ? (effectiveMonthlySavings === 0 ? '' : Math.floor(effectiveMonthlySavings).toString())
                      : (sliderMonthlySavings[0] === 0 ? '' : Math.floor(sliderMonthlySavings[0]).toString())
                    }
                    onChange={(e) => {
                      if (!useAutoMode) {
                        const val = e.target.value;
                        if (val === '') {
                          ...
                          if (!isNaN(num) && num >= 0) {
                            ...
                            setDynamicMonthlySavingsMax(Math.max(defaultMonthlySavingsMax, Math.ceil(clampedValue * 1.2)));
                          }
                        }
                      }
                    }}
                ...
                <Slider
                  value={useAutoMode ? [effectiveMonthlySavings] : sliderMonthlySavings}
                  onValueChange={(vals) => {
                    if (!useAutoMode && !isSlidingRef.current) {
              ...
              <div>
                ...
                <p className="text-xs text-primary/60 mb-3">
                      {useAutoMode ? (
                    <>
                      Auto: {(autoReturns.nomAvailable * 100).toFixed(1)}% 
                      {useInvestedCapital 
                        ? ' (viktat snitt från aktier & fonder)' 
                ...
                {!useAutoMode && (
                  ...
                  <div className="flex items-center gap-2 md:gap-3 mb-2">
                    <Input
                      ...
                      inputMode="decimal"
                      value={useAutoMode 
                        ? (effectiveReturnNominal * 100).toFixed(1)
                        : sliderReturn[0].toFixed(1)
                      }
                      onChange={(e) => {
                        if (!useAutoMode) {
                          const val = e.target.value;
                ...
                <Slider
                  value={useAutoMode ? [effectiveReturnNominal * 100] : sliderReturn}
                  onValueChange={(vals) => {
                    if (!useAutoMode && !isSlidingRef.current) {
                ...
                />
                <div className="text-xs text-gray-500">
                  {useInflation ? 'Real avkastning' : 'Nominell avkastning'}: {useInflation ? (effectiveReturnReal >= 0 ? '+' : '') + (effectiveReturnReal * 100).toFixed(1) : (effectiveReturnNominal * 100).toFixed(1)}%/år
                </div>
              </div>
              ...
              <div>
                ...
                />
                <div className="text-xs text-gray-500">
                  {sliderYears[0]} {sliderYears[0] === 1 ? 'år' : 'år'}
                </div>
              </div>
            </div>
                          ...
                          <Slider
                            ...
                            onValueCommit={(vals) => {
                              ...
                              try {
                                const updated = [...savingsPlans];
                                const newValue = vals[0];
                                const currentMax = planMaxValues[plan.id]?.monthlySavings || 50000;
                                const clampedValue = Math.max(0, Math.min(newValue, Math.min(currentMax, 999999999)));
                                updated[idx].monthlySavings = clampedValue;
                                setSavingsPlans(updated);
                                // Uppdatera grafen omedelbart när reglaget släpps
